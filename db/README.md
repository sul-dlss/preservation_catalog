...
* <sub>The "1" (or single line) flush with the table being connected to indicates up to 1 of the referent (e.g. a `PreservedObject` might have zero or one `preserved_objects_primary_moabs` entry, but a `preserved_objects_primary_moabs` row has exactly one `PreservedObject`).</sub>
...
* A `PreservedObject` represents the master record for a druid, tying together its `complete_moabs` and `zipped_moab_versions`. It also holds some high level summary info that applies to all of the related complete moabs.
...
* A `CompleteMoab` represents a physical copy of a `PreservedObject`, stored on premises as a Moab (containing all versions for the druid) and accessed via Ceph mount.  Multiple `CompleteMoab` instances can belong to the same `PreservedObject` instance (on different storage roots).
  ...
  * `version`: should be the same as `PreservedObject` current version for the primary, may not match for in non-primary copies (lagging `current_version` would be more expected).
* A `MoabStorageRoot` represents a physical storage location (on premises) on which a `CompleteMoab` resides, e.g., a single Ceph or NFS mounted storage root.  The `MoabStorageRoot` fields include info about:
...
* `PreservedObjectsPrimaryMoab`: indicates the "primary" copy of a `CompleteMoab` for a `PreservedObject` (i.e. the one Preservation Robots should keep up to date).  If a druid has multiple Moabs, Pres Robots will query Pres Cat (via REST API) to determine which copy to update.
...
* In the unlikely event that you're tempted to pro-actively do row-locking, e.g. due to concern about multiple processes updating a shared resource (e.g. if multiple processes were crawling the same storage root and doing moab validation), the Postgres docs seems to advise against that.  Instead, specifying transaction isolation level seems to be recommended as the more robust and performant approach.
  ...
  * E.g. you *probably shouldn't* do it this way: http://api.rubyonrails.org/classes/ActiveRecord/Locking/Pessimistic.html
...
druids_with_replication_issues =
  ZipPart.
    joins(zipped_moab_version: [{ preserved_object: [:complete_moab] }, :zip_endpoint]).
    where.not(zip_parts: { status: 'ok' }).
    group(:druid, :endpoint_name).
    pluck(:druid,
          'SUM(zip_parts.size) AS zmv_size',
          'max(complete_moabs.size) AS moab_size', # max(complete_moabs.size) -- heuristic is that biggest moab is usually latest version is usually most up to date, since things mostly work
          "(CASE WHEN SUM(zip_parts.size) >= max(complete_moabs.size) THEN 'zmv >= cm' ELSE 'cm larger!' END) AS size_cmp",
          "SUM(zip_parts.size) - max(complete_moabs.size) AS zmv_size_diff",
          :endpoint_name,
          'COUNT(zip_parts.id) parts_count',
          'ARRAY_AGG(zip_parts.status) part_statuses')
...
druids_with_replication_issues_human = druids_with_replication_issues.map do |result|
  [
    ...
    result[3], # size_cmp
    number_to_human_size(result[4]), # zmv_size_diff
    result[5], # endpoint_name
    result[6], # zip part count
    # result[7].map { |status_int| ZipPart.statuses.key(status_int) } # status for all parts on EP
    result[7].map { |status_int| ZipPart.statuses.key(status_int) }.uniq
  ]
end
```
