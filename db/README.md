...
- <sub>The "1" (or single line) flush with the table being connected to indicates up to 1 of the referent (e.g. a `PreservedObject` might have zero or one `preserved_objects_primary_moab` entry, but a `preserved_objects_primary_moab` row has exactly one `PreservedObject`).</sub>
