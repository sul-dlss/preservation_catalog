...
#### how many moabs on each storage root `status != 'ok'`?
```ruby
# example AR query
[12] pry(main)> CompleteMoab.joins(:preserved_object, :endpoint).where.not(status: CompleteMoab::OK_STATUS).group(:status, :storage_location).order('complete_moabs.status asc, endpoints.storage_location asc').pluck('complete_moabs.status, endpoints.storage_location, count(preserved_objects.druid)')
```
```sql
-- example sql produced by above AR query
