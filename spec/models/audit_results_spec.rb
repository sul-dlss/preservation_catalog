...
RSpec.describe AuditResults do
  ...
  describe '#remove_db_updated_results' do
    before do
      ...
      code = AuditResults::MOAB_RECORD_STATUS_CHANGED
    ...
    it 'removes results matching DB_UPDATED_CODES' do
      ...
      expect(audit_results.results).not_to include(a_hash_including(AuditResults::MOAB_RECORD_STATUS_CHANGED))
    end

    it 'keeps results not matching DB_UPDATED_CODES' do
      audit_results.remove_db_updated_results
      expect(audit_results.results).to include(a_hash_including(AuditResults::DB_VERSIONS_DISAGREE))
  ...
  describe '#contains_result_code?' do
    it 'returns true if the result code is there, false if not' do
      expect(audit_results.results.size).to eq 0
      added_code = AuditResults::DB_VERSIONS_DISAGREE
  ...
  describe 'result array subsets' do
    let(:result_code) { AuditResults::MOAB_RECORD_STATUS_CHANGED }
  ...
  describe '#status_changed_to_ok?' do
    it 'returns true if the new status is ok' do
      added_code = AuditResults::MOAB_RECORD_STATUS_CHANGED
  ...
  describe '#to_json' do
    it 'returns valid JSON for the current results' do
      audit_results.add_result(AuditResults::DB_VERSIONS_DISAGREE, moab_record_version: 1, po_version: 2)
      ...
      expect(json_parsed['results'].first.keys).to eq [AuditResults::DB_VERSIONS_DISAGREE.to_s]
      expect(json_parsed['results'].first[AuditResults::DB_VERSIONS_DISAGREE.to_s]).to eq exp_msg
