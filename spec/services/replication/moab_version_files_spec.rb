...
RSpec.describe Replication::MoabVersionFiles do
  subject(:moab_version_files) { described_class.new(moab_version_root: pathfinder.moab_version_root) }
  ...
  describe '#moab_version_size' do
    ...
    context 'when the moab_version_root is not found' do
      let(:storage_location) { 'spec/fixtures/storage_rootZZZZZZZZZZZZZZ99/sdr2objects' }

      it 'raises Replication::Errors::MoabVersionDirectoryNotFound' do
        expect { moab_version_files.size }.to raise_error(Replication::Errors::MoabVersionDirectoryNotFound)
      end
    end
  end
end
