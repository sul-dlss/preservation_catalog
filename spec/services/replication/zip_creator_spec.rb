...
RSpec.describe Replication::ZipCreator do
  ...
  describe '#create!' do
    context 'when zip file is successfully created' do
      after do
        Replication::ZipPartCleaner.clean!(pathfinder:)
      end

      it 'creates the zip file and the md5 sidecar file' do
        expect { creator.create! }.to change(pathfinder, :all_file_paths)
                                  .from([])
          .to([instance_of(Pathname), instance_of(Pathname)])
      end
    end
