...
RSpec.describe Replication::ZipPartCompletenessChecker do
  ...
  describe '#complete?' do
    ...
    context 'when zip is complete' do
      before do
        Replication::ZipCreator.create!(pathfinder:)
      end
    ...
    context 'when total part size is smaller than the Moab version size' do
      before do
        Replication::ZipCreator.create!(pathfinder:)
        allow(Replication::MoabVersionFiles).to receive(:new)
                                            .and_return(instance_double(Replication::MoabVersionFiles, size: 1_000_000))
      end
