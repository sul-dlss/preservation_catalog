  ...
  describe ".check_moab_to_catalog_existence" do
    ...
    context "determine create or update method with expected values" do 
      ...
      it "call #create if object does not exist" do
        ...
        subject
        expected_argument_list.each { |arg_hash| expect(arg_hash[:po_handler]).to have_received(:create) }
      end
      it "call #update if object exists" do
        ...
        subject
        expected_argument_list.each { |arg_hash| expect(arg_hash[:po_handler]).to have_received(:update) }
      end
    end
