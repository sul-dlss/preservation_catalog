...
class PreservedObject < ApplicationRecord
  ...
  # given a version, create any PreservedCopy records for that version which don't yet exist for zip
  ...
  def create_archive_preserved_copies(archive_vers)
    ...
    params = Endpoint.which_need_archive_copy(druid, archive_vers).map do |zep|
      { version: archive_vers, endpoint: zep, status: PreservedCopy::UNREPLICATED_STATUS }
    end
