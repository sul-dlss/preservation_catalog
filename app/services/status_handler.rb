...
##
# service class for updating status in Results and MoabRecord
class StatusHandler
  delegate :add_result, to: :results
  ...
  def initialize(results:, moab_record:)
    @results = results
  ...
  def update_moab_record_status(new_status)
    ...
    if moab_record.status_changed?
      add_result(
        Results::MOAB_RECORD_STATUS_CHANGED, old_status: moab_record.status_was, new_status: moab_record.status
      )
    end

    moab_record.status_details = results.to_s
  end

  def mark_moab_not_found
    add_result(Results::MOAB_NOT_FOUND,
  ...
  def validate_moab_on_storage_and_set_status(found_expected_version:, moab_on_storage_validator:, caller_validates_checksums: false)
    ...
    if results.contains_result_code?(Results::MOAB_CHECKSUM_VALID)
  ...
  attr_reader :moab_record, :results
end
