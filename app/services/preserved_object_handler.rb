...
class PreservedObjectHandler
  ...
  def confirm_version
    if invalid?
      ...
      if endpoint.endpoint_type.endpoint_class == 'online'
        ...
        # TODO: do some confirmation with archive version
      end
    end
  ...
  def confirm_online_version
    transaction_ok = with_active_record_transaction_and_rescue do
      ...
      if incoming_version > po_db_object.current_version && po_db_object.current_version == pc_db_object.version
        handler_results.add_result(PreservedObjectHandlerResults::ARG_VERSION_GREATER_THAN_DB_OBJECT, po_db_object.class.name)
        po_db_object.current_version = incoming_version
        handler_results.add_result(PreservedObjectHandlerResults::ARG_VERSION_GREATER_THAN_DB_OBJECT, pc_db_object.class.name)
        if moab_validation_errors.empty?
          update_status(pc_db_object, PreservedCopy::OK_STATUS)
        else
          update_status(pc_db_object, PreservedCopy::INVALID_MOAB_STATUS)
        end
        update_preserved_copy_version_etc(pc_db_object, incoming_version, incoming_size, true)
      elsif incoming_version == po_db_object.current_version && po_db_object.current_version == pc_db_object.version
        handler_results.add_result(PreservedObjectHandlerResults::VERSION_MATCHES, po_db_object.class.name)
        handler_results.add_result(PreservedObjectHandlerResults::VERSION_MATCHES, pc_db_object.class.name)
      else
        handler_results.add_result(PreservedObjectHandlerResults::ARG_VERSION_LESS_THAN_DB_OBJECT, po_db_object.class.name)
        update_status(pc_db_object, PreservedCopy::EXPECTED_VERS_NOT_FOUND_ON_STORAGE_STATUS)
        handler_results.add_result(PreservedObjectHandlerResults::ARG_VERSION_LESS_THAN_DB_OBJECT, pc_db_object.class.name)
      end
      update_db_object(pc_db_object)
      update_db_object(po_db_object)
    end
    handler_results.remove_db_updated_results unless transaction_ok
  end
