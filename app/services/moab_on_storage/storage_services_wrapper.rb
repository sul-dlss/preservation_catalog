...
module MoabOnStorage
  ...
  class StorageServicesWrapper
    ...
    def self.filepath(druid, category, filename, version = nil)
      raise(ArgumentError, "No filename provided to StorageServicesWrapper.filepath for druid #{druid}") if filename.blank?
      err_msg = "category arg must be 'content', 'metadata', or 'manifest' (StorageServicesWrapper.filepath for druid #{druid})"
